...
class Scene1ArchAndWrite(Scene):
    ...
    def construct(self):
        ...
        sectors_label.next_to(disk_2d, DOWN, buff=0.08)

        self.play(FadeIn(sectors_label, run_time=0.3))
        ...
        self.play(FadeOut(sectors_label, run_time=0.4))
        cap2 = show_caption(
            self,
        ...
        code_label_api = Text(
            'WriteFile() <WinAPI>',
            font=FONT, font_size=16, color=RED,
        ).next_to(um[2], RIGHT, buff=0.15)
        ...
        self.play(packet.animate.move_to(um[1][0].get_right() + LEFT * 1.05), run_time=0.35)
        highlight_block(self, um[1], YELLOW, duration=0.15)

        self.play(packet.animate.move_to(um[2][0].get_right() + LEFT * 1.05), run_time=0.35)
        highlight_block(self, um[2], YELLOW, duration=0.15)
        self.play(FadeIn(code_label_api, run_time=0.3))

        # Фаза: User mode chain — пакет движется по левому краю блоков
        for i in range(2, len(um) - 1):
        ...
        self.play(
            plain_packet.animate.next_to(minifilter, RIGHT, buff=0.2),
            run_time=0.2
        )

        lock_group, lock_anims = get_lock_closing_animation(
            plain_packet,
            scale=0.32,
            run_time=4
        )
        ...
        self.play(
            *lock_anims,
            Transform(
                plain_packet,
                create_data_packet('"7F A3 9C D2 ..."', CIPHERTEXT_COLOR, font_size=13).move_to(plain_packet),
            ),
            run_time=4,
        ...
        self.wait(0.5)
        self.play(FadeOut(lock_group), run_time=0.3)
        ...
        self.play(
            cipher_packet.animate
                .scale(0.9)
                .next_to(irp_token, DOWN, buff=0.08),
            run_time=0.45,
            rate_func=rate_functions.ease_out_cubic
        )

        irp_with_data = VGroup(irp_token, cipher_packet)
        self.play(
            irp_with_data.animate.move_to(disk_2d.get_center()),
            run_time=0.6,
        )
        sectors = disk_2d[8]
        ...
        fwrite_result_rect = RoundedRectangle(
            corner_radius=0.08,
            width=2.5,
            height=0.75,
            fill_color=PLAINTEXT_COLOR,
            fill_opacity=0.12,
            stroke_color=PLAINTEXT_COLOR,
            stroke_width=2,
        )

        fwrite_result_text = VGroup(
            Text(
                "fwrite() return:",
                font=FONT,
                font_size=12,
        ...
        self.play(
            FadeIn(fwrite_result_block, shift=LEFT * 0.1),
            run_time=0.5
        )
        ...
        self.play(
            FadeOut(irp_token),
            FadeOut(status_block),
            FadeOut(fwrite_result_block),
            FadeOut(code_label),
        ...
        self.play(irp_token_r.animate.move_to(um[1][0].get_right() + LEFT * 1.05), run_time=0.35)
        highlight_block(self, um[1], YELLOW, duration=0.15)

        self.play(irp_token_r.animate.move_to(um[2][0].get_right() + LEFT * 1.05), run_time=0.35)
        highlight_block(self, um[2], YELLOW, duration=0.15)
        self.play(FadeIn(code_label_r_api, run_time=0.3))
        ...
        cipher_packet_r = create_data_packet('"7F A3 9C D2 ..."', CIPHERTEXT_COLOR, font_size=13)
        cipher_packet_r.next_to(irp_token_r, DOWN, buff=0.08)

        irp_with_data_r = VGroup(irp_token_r, cipher_packet_r)
        self.play(
            irp_with_data_r.animate.move_to(disk_2d.get_center()),
            run_time=0.6,
        )
        self.wait(0.5)
        ...
        for block in reversed(km_stack[2:]):
            self.play(
                irp_with_data_r.animate.move_to(block.get_right() + LEFT * 1.05),
                run_time=0.25,
            )
        self.play(
            irp_with_data_r.animate.move_to(km_stack[1].get_right() + LEFT * 1.05),
        ...
        self.play(
            irp_with_data_r.animate.move_to(minifilter.get_right() + LEFT * 1.05),
            
            run_time=0.4,
        )
        highlight_block(self, minifilter, CIPHERTEXT_COLOR, duration=0.3)
        # self.play(
        #     FadeOut(cipher_packet_r),
        #     run_time=0.25
        # )
        ...
        lock_group, lock_anims = get_lock_opening_animation(
            cipher_packet_r,
            scale=0.32,
            run_time=4.0
        )
        ...
        self.play(
            cipher_packet_r.animate
                .scale(0.9)
                .next_to(irp_token_r, DOWN, buff=0.08),
            run_time=0.45,
            rate_func=rate_functions.ease_out_cubic
        )
        ...
        self.play(FadeOut(code_label_r, run_time=0.3))
        ...
        self.play(FadeOut(irp_with_data_r), FadeOut(cipher_packet_r), FadeOut(result), run_time=0.3)
