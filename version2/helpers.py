...
def create_disk_2d(width=2.8, height=1.0) -> VGroup:
    ...
    # Иконка HDD — три горизонтальные линии
    line_w = width * 0.5
    lines = VGroup()
    for dy in [0.12, 0, -0.12]:
        l = Line(LEFT * line_w / 2, RIGHT * line_w / 2, stroke_color="#666666", stroke_width=1)
        l.shift(UP * dy)
        lines.add(l)
    lines.move_to(body.get_center() + LEFT * (width * 0.15))

    # LED-точка
    dot = Dot(radius=0.04, color="#55ff55").move_to(body.get_center() + RIGHT * (width * 0.3))

    label = Text("HDD / SSD", font=FONT, font_size=10, color=GRAY_B)
    label.next_to(body, DOWN, buff=0.04)

    # Сектора (невидимые, заполняются при записи)
    sectors = VGroup()
    for i in range(6):
        s = RoundedRectangle(
            corner_radius=0.02, width=0.2, height=0.15,
            stroke_color="#555555", stroke_width=0.5, fill_opacity=0,
        )
        sectors.add(s)
    sectors.arrange(RIGHT, buff=0.04)
    sectors.next_to(body, DOWN, buff=0.15)

    return VGroup(body, lines, dot, label, sectors)
